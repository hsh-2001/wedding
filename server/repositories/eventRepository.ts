...
const upsertGuest = async (guest: IUpsertGuestRequest) => {
  const sql = `SELECT * FROM upsert_guest($1, $2, $3, $4, $5, $6, $7, $8, $9)`;
  const params = [guest.id, guest.wedding_id, guest.name, guest.title, guest.phone, guest.email, guest.invitation_code, guest.remark, guest.is_invited];
  const result = await connection.getData(sql, params);
  return ApiResponse.success('Guest upserted successfully', result[0]);
}

const getGuestsByWeddingId = async (weddingId: string, limit = 25, offset = 0, search = '') => {
  const sql = `SELECT * FROM get_guest_by_event_id($1, $2, $3, $4)`;
  const params = [weddingId, limit, offset, search];
